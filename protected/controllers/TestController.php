...
Yii::import('application.modules.fileUploadModel.apiservices.ApiServiceQiniuFileUrl');
...
class TestController extends WebsiteController {
    ...
    public function actionUploadToken() {
        ...
        $output = $fileMgr->getUploadToken($bucket);
     ...
     * 文件保存
     */
    public function actionSavePatientMrFile() {
        if (isset($_POST['appFile'])) {
            $appFile = $_POST['appFile'];
            $patientMgr = new PatientManager();
            $output = $patientMgr->saveAppFile($appFile);
        } else {
            $output = array(
                'status' => 'no',
                'errorCode' => 1,
     ...
     * 查看文件
     */
    public function actionPatientFileList($id) {
        $output = array(
            'status' => 'ok',
            'errorCode' => 0,
            'errorMsg' => 'success',
        ...
        $patientMgr = new PatientManager();
        $models = $patientMgr->loadPatientMRFilesByPatientId($id);
        $apiSvc = new ApiServiceQiniuFileUrl();
        $result = new stdClass();
        foreach ($models as $model) {
            $result->files[] = $apiSvc->getAbsFileUrl($model, 'test/patientFile');
        }
     ...
     * @param type $type
     */
    public function actionPatientFile($uid, $type) {
