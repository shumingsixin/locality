...
class MsgManager {
    ...
    public function sendMailQueue(){
        ...
        $duration = 270;
        $limit = 100;
        $models = MsgQueueEmail::model()->getNeedSendMail($limit);	// limit=$limit
        $emailMgr = new EmailManager();
        $msgQueueLog = new MsgQueueLog();
        ...
        foreach($models as $model){
            ...
            try{
                if($emailMgr->sendMsgQueueEmail($model)){
                    $model->is_sent = 1;
//                    $model->date_read = new CDbExpression('NOW()');
                }
                $model->attempts = $model->attempts + 1;
                $model->last_attempt = date('Y-m-d H:i:s', $timeNow);

                $model->save();
            }catch(CException $ex){
                $msgQueueLog->msg_email_id = $model->id;
                $msgQueueLog->level = 'error';
                $msgQueueLog->category = 'sendMail';
//                $msgQueueLog->subject = $model->id;
                $msgQueueLog->message = $ex->getMessage();
                $msgQueueLog->save();
            }
    ...
    public function createMsgQueueEmail($code, $toEmail, $templateParams, $subject, $senderEmail='noreply@mingyihz.com', $senderName='名医主刀'){
        $template = MsgEmailTemplate::model()->getByCode($code);
        if(!is_object($template)){
            return false;
        }
        $viewFile = $template->view_file;
        $msgQueue = new MsgQueueEmail();
        $msgQueue->subject = $subject;
        $msgQueue->body= $this->renderView($viewFile, $templateParams);
        $msgQueue->max_attempts = 3;
        $msgQueue->template_code = $code;
        $msgQueue->from_email = $senderEmail;
        $msgQueue->from_name = $senderName;
     ...
     * @return int
     */
    public function sendEmailAppBooking2(Booking $model) {
        $view = 'appBookingNew';
    ...
    public function sendEmailDoctorUpdateInfo($data) {
    ...
    private function renderView($view, $params) {
        return Yii::app()->controller->renderPartial($this->mail_view . $view, $params, true); // Make sure to return true since we want t
    }
