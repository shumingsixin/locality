...
class PatientMRFile extends FileUploadModel {
    ...
    public function rules() {
        ...
        return array(
            ...
            array('file_url, thumbnail_url, base_url, remote_domain, remote_file_key', 'length', 'max' => 255),
    ...
    public function initAppModel($appFile) {
        $this->setPatientId($appFile['patientId']);
        $this->setCreatorId($appFile['userId']);
        $reportType = StatCode::MR_REPORTTYPE_MR;
        if (isset($appFile['reportType'])) {
            $reportType = $appFile['reportType'];
        }
        $this->setReportType($reportType);
        $this->setAppFileAttributes($appFile);
    }
